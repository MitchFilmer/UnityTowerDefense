...
public class FireStats : TowerBase
{
    ...
    void Start()
    {
		...
		GetComponent<SphereCollider>().radius = (GetComponent<FireStats>().attackRange) / 2;
	...
	public override bool Upgrade()
	{
		if(base.Upgrade())
		{
			render.material.color -= new Color(0f, 0.3f, 0.3f);
	...
	private void RemoveEnemyFromList()
	{
		...
		{
			Debug.Log(string.Format("Removing {0} from Splash Tower", inactiveEnemy.name));
